...
buildNpmPackage (finalAttrs: {
  ...
  postPatch = ''
    ...
    # Fix TypeScript build error: @google/gemini-cli-devtools not found during build
    # Add TypeScript ignore comment to skip type checking for this import
    if [ -f packages/cli/src/utils/devtoolsService.ts ]; then
      # Add @ts-ignore before the problematic import line
      sed -i '/const mod = await import.*@google\/gemini-cli-devtools/i // @ts-ignore - devtools package built separately' packages/cli/src/utils/devtoolsService.ts || true
    fi
  '';
