    ...
    echo "$binds_json" | jq -r '
      ...
      def autogenerate_comment(dispatcher; params):
        if dispatcher == "killactive" then "Close window"
          ...
          elif (params | test("wpctl.*volume")) then "Volume control"
          elif (params | test("brightness-osd|brightnessctl")) then "Brightness control"
      ...
      def get_category(dispatcher; params):
        if dispatcher == "exec" then
          ...
          elif (params | test("playerctl|wpctl")) then "🔊 Media & Volume"
          elif (params | test("brightness-osd|brightnessctl")) then "🔆 Brightness"
