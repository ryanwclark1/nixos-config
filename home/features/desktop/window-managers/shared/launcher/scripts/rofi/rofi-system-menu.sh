...
show_screenshot_menu() {
  ...
  case "$selection" in
    *Region*)
      if command -v screenshot.sh >/dev/null; then
        screenshot.sh area --satty
      else
        notify-send "Screenshot" "screenshot.sh not available"
      fi
      ;;
    *Window*)
      if command -v screenshot.sh >/dev/null; then
        screenshot.sh window --satty
      else
        notify-send "Screenshot" "screenshot.sh not available"
      fi
      ;;
...
show_audio_menu() {
  ...
  case "$selection" in
    *Switch*)
      if command -v os-audio-switch >/dev/null; then
        os-audio-switch
      else
        notify-send "Audio" "os-audio-switch not available"
      fi
      ;;
    *Mute*)
      if command -v os-audio-volume-mute >/dev/null; then
        os-audio-volume-mute
      else
        notify-send "Audio" "os-audio-volume-mute not available"
      fi
      ;;
    *Down*)
      if command -v os-audio-volume-down >/dev/null; then
        os-audio-volume-down
      else
        notify-send "Audio" "os-audio-volume-down not available"
      fi
      ;;
    *Up*)
      if command -v os-audio-volume-up >/dev/null; then
        os-audio-volume-up
      else
        notify-send "Audio" "os-audio-volume-up not available"
      fi
      ;;
    *Mixer*)
