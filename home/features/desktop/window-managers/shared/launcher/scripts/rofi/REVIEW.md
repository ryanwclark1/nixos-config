...
```bash
# Instead of:
CMD_SETTINGS="$HOME/.config/desktop/window-managers/shared/scripts/rofi/rofi-settings-menu.sh"

# Use:
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CMD_SETTINGS="$SCRIPT_DIR/rofi-settings-menu.sh"
# Or check for existence first
```

### 3. Missing Error Handling
**Affected Scripts**: `rofi-quicklinks.sh`, `rofi-settings-menu.sh`, `rofi-system-menu.sh`
...
- `rofi-system-menu.sh`: Hardcoded theme name
...
### `rofi-system-menu.sh` & `rofi-system-menu-legacy.sh`
