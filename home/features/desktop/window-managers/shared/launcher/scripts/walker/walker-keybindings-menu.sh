...
monitor_height=$(hyprctl monitors -j 2>/dev/null | jq -r '.[] | select(.focused == true) | .height' | head -1 || echo "1080")
menu_height=$((monitor_height * 40 / 100))

if ! build_keymap_cache; then
  echo "Warning: Failed to build keymap cache, continuing with limited functionality" >&2
fi

{
  ...
  parse_bindings |
  prioritize_entries |
  walker --dmenu -p 'Keybindings' --width 800 --height "$menu_height" || true
