...
set -ogq @bg "#{@base00}"           # Background
set -ogq @fg "#{@base05}"           # Foreground
set -ogq @surface_0 "#{@base01}"    # Surface level 0
set -ogq @surface_1 "#{@base02}"    # Surface level 1
set -ogq @surface_2 "#{@base03}"    # Surface level 2
set -ogq @muted "#{@base04}"        # Muted text
set -ogq @subtle "#{@base06}"       # Subtle text
set -ogq @text "#{@base07}"         # High contrast text

# Standard color semantics
set -ogq @error "#{@base08}"        # Red - Error states
set -ogq @warning "#{@base09}"      # Orange - Warning states
set -ogq @attention "#{@base0A}"    # Yellow - Attention
set -ogq @success "#{@base0B}"      # Green - Success states
set -ogq @info "#{@base0C}"         # Cyan - Information
set -ogq @primary "#{@base0D}"      # Blue - Primary accent
set -ogq @secondary "#{@base0E}"    # Magenta - Secondary accent
set -ogq @accent "#{@base0F}"       # Brown - Accent color

# Extended palette
set -ogq @mantle "#{@base10}"       # Darker background
set -ogq @crust "#{@base11}"        # Darkest background
set -ogq @bright_red "#{@base12}"   # Bright red
set -ogq @bright_yellow "#{@base13}" # Bright yellow
set -ogq @bright_green "#{@base14}" # Bright green
set -ogq @bright_cyan "#{@base15}"  # Bright cyan
set -ogq @bright_blue "#{@base16}"  # Bright blue
set -ogq @bright_purple "#{@base17}" # Bright purple

# Theme metadata
EOF
    echo "set -ogq @theme_name \"$theme_name\"" >> "$output_file"
    echo "set -ogq @theme_author \"$theme_author\"" >> "$output_file"
    echo "set -ogq @theme_variant \"$theme_variant\"" >> "$output_file"
}
