...
static_checks() {
  ...
  require_pattern "$launcher_ipc_handler_qml" 'function drunCategoryState\(\)\s*(?::\s*string)?\s*\{\s*return JSON\.stringify\(launcher\.drunCategoryStateObject\(\)\);\s*\}' "drun category IPC payload mapping"
  require_pattern "$launcher_ipc_handler_qml" 'function escapeActionState\(\)\s*(?::\s*string)?\s*\{\s*return JSON\.stringify\(launcher\.escapeActionStateObject\(\)\);\s*\}' "escape action IPC payload mapping"
  require_pattern "$launcher_ipc_handler_qml" 'function diagnosticSetSearchText\(text(?::\s*string)?\)\s*(?::\s*string)?\s*\{\s*return launcher\.diagnosticSetSearchText\(text\);\s*\}' "escape action query setter IPC mapping"
  require_pattern "$launcher_ipc_handler_qml" 'function diagnosticSetDrunCategoryFilter\(categoryKey(?::\s*string)?\)\s*(?::\s*string)?\s*\{\s*return launcher\.diagnosticSetDrunCategoryFilter\(categoryKey\);\s*\}' "escape action category setter IPC mapping"
  require_pattern "$launcher_ipc_handler_qml" 'function diagnosticSetViewport\(widthValue(?::\s*real)?\s*,\s*heightValue(?::\s*real)?\)\s*(?::\s*string)?\s*\{\s*return launcher\.diagnosticSetViewport\(widthValue,\s*heightValue\);\s*\}' "viewport override IPC mapping"
