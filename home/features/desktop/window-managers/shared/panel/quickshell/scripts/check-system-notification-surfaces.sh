...
populate_repo_shell_env() {
  build_repo_shell_env_array repo_shell_env "PATH=${script_dir}:${PATH}" "QS_DISABLE_NOTIFICATION_SERVER=1"
}
...
static_checks() {
  ...
  require_literal "${shell_root_qml}" 'active: Config.isPanelEnabled("notifCenter") && root.notifCenterVisible' "ShellRoot notifCenter lazy-loader activation"
  require_literal "${shell_root_qml}" 'active: Config.isPanelEnabled("systemMonitor") && root.systemMonitorVisible' "ShellRoot systemMonitor lazy-loader activation"
