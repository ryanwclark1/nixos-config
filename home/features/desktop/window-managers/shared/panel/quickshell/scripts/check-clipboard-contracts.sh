...
launcher_content_panel_qml="${repo_root}/src/launcher/LauncherContentPanel.qml"
...
require_literal "$launcher_content_panel_qml" 'onAccepted: modifiers => launcher.handleSearchAccepted(modifiers)' "Launcher search field is wired to the shared Enter handler"
...
require_literal "$clipboard_menu_qml" 'icon: ClipboardHistoryService.loading ? "download.svg" : "arrow-clockwise.svg"' "Clipboard widget exposes a refresh affordance in the header"
...
require_literal "$clipboard_service_qml" 'Quickshell.execDetached(["sh", "-c", "printf '\''%s\\n'\'' \"$1\" | cliphist delete", "sh", line]);' "Clipboard service deletes entries by piping the raw cliphist row"
