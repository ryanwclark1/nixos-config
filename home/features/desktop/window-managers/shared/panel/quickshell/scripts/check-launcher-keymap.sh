...
require_literal "$launcher_qml" 'selectedIndex = Search.jumpSelectionBoundary(filteredItems.length, toEnd);' "selection boundary assignment"
require_literal "$launcher_qml" 'function pageSelection(step) {' "selection paging helper"
require_literal "$launcher_qml" 'var next = Search.pageSelection(filteredItems.length, selectedIndex, step, hudBox.height);' "selection paging size calculation"
