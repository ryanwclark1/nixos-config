...
EOF
    ...
    for rel_dir in settings-* surfaces-*; do
      [[ -d "${output_dir}/${rel_dir}" ]] || continue
      ...
      while IFS= read -r image_path; do
        ...
        printf '        </a>\n'
        printf '      </div>\n'
      done < <(cd "${output_dir}" && find "${rel_dir}" -maxdepth 1 -type f -name '*.png' | sort)
...
usage() {
  cat <<'EOF'
Usage: capture-panel-matrix.sh [--output-dir DIR] [--settings-preset portrait|laptop|wide] [--surface-crop monitor|usable] [--settings-delay SECONDS] [--surface-delay SECONDS] [--settings-deep-scroll-y PX] [--skip-settings] [--skip-surfaces]
