            ...
            LazyLoader {
                ...
                PanelWindow {
                    ...
                    readonly property var screenWorkspaces: {
                        ...
                        if (!screenOutput || all.length === 0)
                            return all;
                        var filtered = all.filter(function (ws) {
                            return ws.output === screenOutput;
                        });
                                        ...
                                        delegate: Rectangle {
                                            ...
                                            readonly property var wsWindows: {
                                                ...
                                                return all.filter(function (w) {
                                                    return (w.title || "").toLowerCase().indexOf(root.searchQuery) !== -1 || (w.app_id || "").toLowerCase().indexOf(root.searchQuery) !== -1;
                                                });
                                            }
                                                                ...
                                                                MouseArea {
                                                                    ...
                                                                    onClicked: mouse => {
                                                                        if (mouse.button === Qt.RightButton) {
                                                                            windowContextMenu.model = root.buildContextModel(modelData, wsColumn.ws, overviewWindow.screenWorkspaces);
