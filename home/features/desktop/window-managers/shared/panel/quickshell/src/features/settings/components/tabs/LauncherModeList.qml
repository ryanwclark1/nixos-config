...
Column {
    ...
    Repeater {
        ...
        delegate: SettingsReorderRow {
            ...
            reorderState: root.reorderState
            listId: root.listId
            itemId: String(modeRow.modelData || "")
            rowIndex: modeRow.index
            itemCount: root.modeModel.length
            listItem: root
            compactMode: root.compactMode
            minimumHeight: root.compactMode ? 82 : 54
            beginDragFn: root.beginDragFn
            moveDraggedFn: function(listId, targetIndex) {
                if (root.moveDraggedFn)
                    return root.moveDraggedFn(targetIndex);
                return false;
            }
            clearDragStateFn: root.clearDragStateFn
            dropIndexFn: root.dropIndexFn
            ...
            Flow {
                ...
                SettingsReorderButtons {
                    moveUpEnabled: modeRow.index > 0
                    moveDownEnabled: modeRow.index < (root.modeModel.length - 1)
                    onMoveUp: if (root.moveModeFn) root.moveModeFn(modeRow.modelData, -1)
                    onMoveDown: if (root.moveModeFn) root.moveModeFn(modeRow.modelData, 1)
                }
