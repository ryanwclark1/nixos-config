...

{
  ...
  home.packages = with pkgs; [
    # Rofi-based system menu script
    (writeShellScriptBin "system-menu-rofi" ''
      PATH="${pkgs.rofi-wayland}/bin:${pkgs.coreutils}/bin:${pkgs.libnotify}/bin:${pkgs.systemd}/bin:${pkgs.kitty}/bin:$PATH"
      ...
      show_network_menu() {
        ...
        case "$selection" in
        *WiFi*) ${pkgs.kitty}/bin/kitty -e ${pkgs.networkmanager}/bin/nmtui ;;
        *Bluetooth*) ${pkgs.blueberry}/bin/blueberry ;;
        *Settings*) ${pkgs.gnome-control-center}/bin/gnome-control-center network 2>/dev/null || show_network_menu ;;
      ...
      show_settings_menu() {
        ...
        *Appearance*) ${pkgs.gnome-control-center}/bin/gnome-control-center appearance 2>/dev/null || notify-send "Settings" "Appearance settings not available" ;;
        *) [[ -n "$selection" ]] && show_main_menu ;;
        esac
      }
      ...
      show_toggle_menu() {
        ...
        *Night*)
          if pgrep -x hyprsunset >/dev/null; then
            ...
            ${pkgs.hyprsunset}/bin/hyprsunset & notify-send "🌙 Night Light" "Enabled" -t 2000
          fi
          show_toggle_menu
          ;;
        *) [[ -n "$selection" ]] && show_main_menu ;;
      ...
      show_utilities_menu() {
        ...
        *Clipboard*) ${pkgs.rofi-wayland}/bin/rofi -modi "clipboard:greenclip print" -show clipboard -theme android_notification ;;
        *) [[ -n "$selection" ]] && show_main_menu ;;
        esac
      }
    ...
    (writeShellScriptBin "rofi-power" ''
      system-menu-rofi power
    '')
    
    (writeShellScriptBin "rofi-capture" ''
      system-menu-rofi capture
    '')
    
    (writeShellScriptBin "rofi-settings" ''
      system-menu-rofi settings
    '')
  ];
}