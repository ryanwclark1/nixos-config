...
def clean_params(params: list) -> list:
    # For items in the params list remove characters all characters between and including /nix/store and /bin/
    return [re.sub(r"\/nix\/store\/.*?\/bin\/", "", param) for param in params]
...
def get_binds_recursive(current_content, scope):
    ...
    while reading_line < len(content_lines): # TODO: Adjust condition
        ...
        if ((heading_search_result != None) and (heading_search_result.start() == 0)): # Found title
            ...
            line == ""
            or not line.lstrip().startswith("bind")
            or line.lstrip().startswith("binds")
        ):  # Comment, ignore
            pass
