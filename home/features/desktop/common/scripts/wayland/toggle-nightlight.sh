...
ensure_daemon_running() {
    if ! is_daemon_running; then
        ...
        if ! command -v uwsm >/dev/null 2>&1; then
            log "Error: uwsm command not found"
            notify "Error" "uwsm command not found. Cannot start $DAEMON_NAME."
            exit 1
        fi
        ...
        if setsid uwsm app -- "$DAEMON_NAME" >/dev/null 2>&1 & then
            ...
            log "Error: Failed to start $DAEMON_NAME"
            notify "Error" "Failed to start $DAEMON_NAME"
            exit 1
        fi
    fi
}
