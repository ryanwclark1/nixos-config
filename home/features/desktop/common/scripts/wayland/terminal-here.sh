...
# Supports multiple Wayland compositors and terminal emulators with fallback

set -euo pipefail

# Configuration
TERMINAL="${WAYLAND_TERMINAL:-${HYPR_TERMINAL:-}}"
NOTIFICATION_TIMEOUT="${WAYLAND_NOTIFICATION_TIMEOUT:-${HYPR_NOTIFICATION_TIMEOUT:-2000}}"
...
detect_terminal() {
    ...
    for term in ghostty alacritty kitty wezterm foot gnome-terminal konsole xterm; do
...
get_cwd() {
    if command -v terminal-cwd >/dev/null 2>&1; then
        ...
        # Fallback: try to get from active window using compositor-specific methods
        local terminal_pid
        ...
        # Try Sway as fallback
            ...
            terminal_pid=$(swaymsg -t get_tree 2>/dev/null | jq -r '.. | select(.focused?) | .pid // empty' 2>/dev/null | head -n1 || echo "")
        fi
...
main() {
    ...
    if [[ -z "$terminal" ]]; then
        ...
        echo "Error: No terminal emulator found. Please install ghostty, alacritty, kitty, wezterm, foot, gnome-terminal, konsole, or xterm." >&2
        exit 1
    fi
