...
screensaver_in_focus() {
  hyprctl activewindow -j 2>/dev/null | jq -e '.class == "org.os.screensaver"' >/dev/null 2>&1
}

exit_screensaver() {
  hyprctl keyword cursor:invisible false || true
  pkill -x tte 2>/dev/null || true
  pkill -f org.os.screensaver 2>/dev/null || true
  exit 0
}
...
while true; do
  tte -i ~/.config/os/branding/screensaver.txt \
