...
# -----------------------------------------------------
# System Screenshot Wrapper Script
# Maintains compatibility with os-cmd-screenshot interface
# -----------------------------------------------------
#
# This wrapper calls screenshot.sh (in wayland directory) with mode mapping.
# For direct access to screenshot features, use wayland/scripts/screenshot.sh
#
# See SCREENRECORDING.md for detailed documentation
# -----------------------------------------------------
...
# Get script directory to find screenshot.sh in wayland directory
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Prefer wayland directory location (primary location)
SCREENSHOT_SCRIPT="${SCRIPT_DIR}/../wayland/scripts/screenshot.sh"

# Fallback to installed wayland location
...
if [[ -z "$SCREENSHOT_SCRIPT" || ! -f "$SCREENSHOT_SCRIPT" ]]; then
  notify-send "Screenshot Error" "screenshot.sh not found in wayland directory" -u critical -t 3000
  exit 1
fi
