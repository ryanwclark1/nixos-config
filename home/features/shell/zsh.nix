...
let
  gastownEnabled = config.features.agent-desk.gastown.enable;
  gastownShellBlock =
    if gastownEnabled then
      ''
      ...
      ''
        # Gas Town is remote-only on this host.
...
in
{
  programs.zsh = {
    ...
    loginExtra = ''
      ${lib.optionalString (!gastownEnabled) "unset GT_TOWN_ROOT GT_RIG"}
