...
get_hostname_icon_command() {
    ...
    if command -v get_forceline_path >/dev/null 2>&1; then
        script_path="$(get_forceline_path "modules/hostname/hostname_icon.sh")"
    else
        script_path="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/hostname_icon.sh"
    fi
    echo "$script_path"
}
...
generate_hostname_commands() {
    ...
    if [[ "$show_icon" == "yes" ]]; then
        ...
        if command -v get_forceline_path >/dev/null 2>&1; then
            icon_script_path="$(get_forceline_path "modules/hostname/hostname_icon.sh")"
        else
            icon_script_path="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/hostname_icon.sh"
        fi
