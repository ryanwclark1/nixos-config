...
get_battery_icon() {
    local percentage=$(get_tmux_option "@_battery_percentage" "")
    local status=$(get_tmux_option "@_battery_status" "")
    ...
    if [ -z "$percentage" ]; then
        ...
        tmux set-option -g "@_battery_percentage" "$percentage"
    fi
    
    if [ -z "$status" ]; then
        status=$(battery_status)
        tmux set-option -g "@_battery_status" "$status"
    fi
