    ...
    case "$format_type" in
        "shortened")
            ...
            if command -v get_forceline_path >/dev/null 2>&1; then
                shortener_script="$(get_forceline_path "modules/directory/shorten_path.sh")"
            else
                shortener_script="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/shorten_path.sh"
            fi
        ...
        "with_icon")
            ...
            local icon_script
            if command -v get_forceline_path >/dev/null 2>&1; then
                icon_script="$(get_forceline_path "modules/directory/directory_icon.sh")"
            else
                icon_script="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/directory_icon.sh"
            fi
