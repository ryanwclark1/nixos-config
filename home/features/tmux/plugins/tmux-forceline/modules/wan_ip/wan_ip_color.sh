...
if [[ -f "$UTILS_DIR/common.sh" ]]; then
    ...
    HELPERS_PATH="$(get_forceline_path "modules/wan_ip/wan_ip_helpers.sh")"
    source "$HELPERS_PATH"
else
...
main() {
    ...
    online_fg=$(get_tmux_option "@forceline_wan_ip_online_fg" "#{@fg}")
    online_bg=$(get_tmux_option "@forceline_wan_ip_online_bg" "#{@success}")
    cached_fg=$(get_tmux_option "@forceline_wan_ip_cached_fg" "#{@fg}")
    cached_bg=$(get_tmux_option "@forceline_wan_ip_cached_bg" "#{@surface_0}")
    stale_fg=$(get_tmux_option "@forceline_wan_ip_stale_fg" "#{@base00}")
    stale_bg=$(get_tmux_option "@forceline_wan_ip_stale_bg" "#{@warning}")
    offline_fg=$(get_tmux_option "@forceline_wan_ip_offline_fg" "#{@base00}")
    offline_bg=$(get_tmux_option "@forceline_wan_ip_offline_bg" "#{@error}")
