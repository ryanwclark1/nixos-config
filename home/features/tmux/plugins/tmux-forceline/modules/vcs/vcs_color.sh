...
if [[ -f "$UTILS_DIR/common.sh" ]]; then
    ...
    HELPERS_PATH="$(get_forceline_path "modules/vcs/vcs_helpers.sh")"
    source "$HELPERS_PATH"
else
...
main() {
    ...
    clean_fg=$(get_tmux_option "@forceline_vcs_clean_fg" "#{@fg}")
    clean_bg=$(get_tmux_option "@forceline_vcs_clean_bg" "#{@success}")
    staged_fg=$(get_tmux_option "@forceline_vcs_staged_fg" "#{@base00}")
    staged_bg=$(get_tmux_option "@forceline_vcs_staged_bg" "#{@info}")
    diverged_fg=$(get_tmux_option "@forceline_vcs_diverged_fg" "#{@base00}")
    diverged_bg=$(get_tmux_option "@forceline_vcs_diverged_bg" "#{@warning}")
    dirty_fg=$(get_tmux_option "@forceline_vcs_dirty_fg" "#{@base00}")
    dirty_bg=$(get_tmux_option "@forceline_vcs_dirty_bg" "#{@error}")
